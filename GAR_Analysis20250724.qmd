...
height_data = read.csv("DATA/heights.csv")
...
height_data <- height_data %>%
  mutate(
    Animal = str_match(Image, "pilot_(\\d+)_([0-9]+)")[,2:3] %>% apply(1, paste0, collapse = ""),
    slice = str_match(Image, "s(\\d+)")[,2],
    hemisphere = str_match(Image, "h(\\d+)")[,2],
    slice = as.integer(slice),
    hemisphere = as.integer(hemisphere)
    )


summary_data <- height_data %>%
  left_join(Detections_summary, by = c("Animal", "slice", "hemisphere"))
```
